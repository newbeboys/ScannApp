...
4. **Jangan tambah dependency AI cloud berbayar** untuk fitur perbaikan gambar — baik "Perbaiki Pencahayaan" versi klasik maupun "AI Enhance" versi model — tanpa konfirmasi eksplisit. Ini keputusan sadar karena isu biaya (lihat PRD Bagian 4).
...
- **Export PNG** (ditetapkan 23 Agustus 2026 sore, **mengganti** PRD v2 Bagian 3 yang menaruh PNG di kolom Pro): tersedia untuk **semua tier**. ~~Yang tetap Pro dari baris "Export format" itu hanya **DOCX** dan **kontrol level kompresi manual**.~~ — **kontrol level kompresi ikut dibuka 25 Agustus 2026**; seluruh baris "Export format" kini semua tier, termasuk DOCX kalau nanti dibuat.
- **Level kompresi ekspor** (**semua tier** sejak 25 Agustus 2026, sebelumnya Pro): **4 takik**, bukan slider bebas 0–100 — Kecil (q 0.55 / 1600px), Standar (0.75 / 2400px), Tinggi (0.88 / 3200px), Maksimal (0.95 / 4000px). Standar tetap bawaannya, jadi dokumen yang sudah ada tidak berubah hasilnya. `resolveCompressionLevel()` masih ada tapi tidak lagi menanyakan tier — tugasnya sekarang cuma menjaga nilai rusak/asing dari `localStorage` supaya jatuh ke Standar. Angka-angka ini teknis, bukan angka bisnis — boleh disetel ulang tanpa bertanya.
- **Anotasi, tanda tangan, pisah dokumen & ekspor banyak dokumen** (ditetapkan 25 Agustus 2026 setelah uji device, **mengganti** PRD v2 Bagian 3 dan baris "Annotate dan tanda tangan digital tetap Pro-exclusive" di atas): keempatnya tersedia untuk **semua tier**, Basic maupun Pro. Gerbangnya dilepas dari library juga (`setPageMarks`, `saveSplitScan`, `exportDocumentsBatch`), bukan cuma dari UI. Beberapa jam kemudian **kontrol level kompresi manual dan ekspor DOCX menyusul** ke semua tier atas permintaan Boss Ali yang sama — `canChooseCompression` dihapus, `resolveCompressionLevel` tidak lagi menerima tier, dan `BASIC_COMPRESSION` diganti nama jadi `STANDARD_COMPRESSION` karena "yang didapat Basic" sudah tidak berarti apa-apa. ~~DOCX belum ada kodenya sama sekali.~~ Sudah dibuat 25 Agustus 2026 malam. Keputusan tier-nya: ~~saat dibuat bersama OCR, langsung semua tier~~ — **dikoreksi 25 Agustus 2026 malam**, lihat baris **OCR & ekspor DOCX** di bawah.
- **Apa yang masih dijual Pro setelah semua pembatalan** (per 25 Agustus 2026): **bebas iklan, tanpa watermark di PDF, merge tanpa batas halaman, kuota storage lebih besar**, dan nanti **OCR**. Itu juga persis empat baris yang ditampilkan `UpgradeScreen`, jadi paywall-nya tidak perlu diubah. Pola yang sama dengan tiga pembatalan sebelumnya: **dasar untuk semua, Pro untuk kendali & mutu**. Satu-satunya tempat tier masih menyentuh jalur ekspor adalah `shouldWatermark()`.
- **OCR & ekspor DOCX keduanya Pro** (ditetapkan Boss Ali 25 Agustus 2026 malam, **mengoreksi** baris "DOCX semua tier" di atas): dua baris di bagian ini tidak bisa keduanya benar — OCR ditulis sebagai nilai jual Pro, tapi DOCX ditulis semua tier, padahal DOCX yang berguna **isinya hasil OCR**. DOCX semua tier berarti mesin OCR-nya bocor lewat pintu belakang; kalau tidak, Basic dapat DOCX berisi gambar tertempel, yang justru alasan DOCX ditunda dulu. Diputuskan: **keduanya Pro**. Ini membalik arah tiga pembatalan gerbang Pro sebelumnya, dan itu disengaja — pola "dasar untuk semua, Pro untuk kendali & mutu" tetap berlaku, karena reorder/filter/PNG/anotasi/pisah itu soal **akses ke dokumen sendiri**, sedangkan OCR adalah **mesin baru** yang mengubah gambar jadi teks. Gerbangnya ditegakkan di `recognizeDocument()`, bukan di UI. Teks yang **sudah** dikenali tetap dipakai saat ekspor walau tier turun ke Basic: gerbangnya di titik mesin dijalankan, bukan di titik data milik user sendiri dibaca. Engine-nya `@capacitor-mlkit/text-recognition` (on-device, model dibundel, tidak pernah butuh jaringan); empat model skrip non-Latin dibuang dari APK lewat `exclude` di `android/app/build.gradle` (±8,6 MB). **Keduanya sudah jadi** per 25 Agustus 2026 malam — `ExportFormat` kini `pdf | jpg | png | docx`, DOCX ditulis sendiri (ZIP metode STORE + OOXML minimal, nol dependency baru). Lihat `docs/superpowers/specs/2026-08-25-fase6-ocr-docx-design.md`.
...
- **Tujuan ekspor: Bagikan atau Simpan ke HP, tidak pernah dua-duanya** (keputusan Boss Ali 27 Agustus 2026, setelah uji device): lembar Ekspor punya sakelar **Tujuan**, bawaannya **Bagikan**. "Bagikan" menulis ke cache privat aplikasi lalu membuka share sheet — **membatalkan share sheet tidak meninggalkan berkas apa pun di HP**. "Simpan ke HP" menulis ke folder Documents **tanpa** membuka share sheet, dan **tidak pernah menimpa** berkas yang sudah ada (namanya dinaikkan jadi `Nota (2).pdf`). Jangan kembalikan perilaku lama "tulis ke Documents dulu, baru share": itu yang membuat berkas tersimpan walau user membatalkan, **dan** yang memicu `EACCES` — scoped storage Android 11+ tidak mengizinkan membuka ulang berkas di Documents yang bukan milik install yang sekarang, dan kepemilikan itu hilang tiap aplikasi dipasang ulang. Jangan pula menggantinya dengan "tulis dulu, hapus kalau dibatalkan": sebagian OEM menghentikan Activity saat share sheet tampil, sehingga plugin Share melapor sukses walau user membatalkan.
...
- **`REVENUECAT_SECRET_API_KEY` — BELUM ADA, perlu ditambahkan Boss Ali** (per 5 September 2026). Dipakai `request-account-deletion` untuk menanyakan `GET /v1/subscribers/{app_user_id}` ke RevenueCat sebelum menjadwalkan hapus akun. Ambil dari dashboard RevenueCat → Project settings → API keys → **Secret API keys** (v1); ini **bukan** public SDK key `VITE_REVENUECAT_ANDROID_KEY` dan **bukan** `REVENUECAT_WEBHOOK_SECRET`. Selama kosong, fungsi tidak mati — ia jatuh ke cadangan `profiles.tier`/`pro_plan` dan mencatat `revenuecat_key_missing` di log, sengaja begitu karena Google Play mewajibkan jalur hapus akun tetap tersedia.
