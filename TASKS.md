...
- [ ] **Verifikasi `assembleRelease`/`bundleRelease` sungguhan lewat CI** (`build-aab.yml`, `workflow_dispatch` di branch ini) — runner GitHub jauh lebih lega RAM-nya daripada mesin dev, jadi ini pembuktian yang lebih representatif untuk build yang sungguh dikirim ke Play Store ketimbang memaksakan di mesin 3GB.
