...
- [~] Uji limit merge dokumen Basic (20 halaman) & quota storage R2 per tier (100MB/500MB/1GB) sesuai angka final — sisi kode sudah dituntaskan 1 September 2026: `checkMergeAllowed`/angka 20 halaman sudah lama teruji, tapi titik penegakannya sendiri (`mergeDocuments`, yang benar-benar melempar error & tidak pernah menyentuh storage saat melebihi limit) belum punya test — ditambahkan di `documentMerge.test.ts`. Sisi quota R2 (`fitsInQuota`/`quotaBytesFor`/`QUOTA_BYTES`) sudah lama teruji pas dengan angka final, dan wiring-nya di `confirm-upload/index.ts` ditinjau ulang: ukuran diukur nyata dari R2 (bukan klaim client), bukan mock. Yang **belum**: uji hidup di project Supabase+R2 sungguhan (unggah sampai lewat kuota lalu cek ditolak 409) — pola sama seperti checklist "Uji device" Fase 8, perlu Boss Ali di HP/akun test nyata, bukan sesuatu yang bisa disimulasikan dari unit test.
