...
- Autentikasi: **HMAC webhook signing** dari RevenueCat, bukan bearer token di header `Authorization`. Header `X-RevenueCat-Webhook-Signature` berbentuk `t=<unix_seconds>,v1=<hmac_sha256_hex>`; signature dihitung dari `"{t}.{raw_body}"` memakai signing secret (`REVENUECAT_WEBHOOK_SECRET`, di-set Boss Ali dari toggle "HMAC webhook signing" di dashboard RevenueCat + Supabase Edge Function Secrets). Diputuskan setelah audit 22 Agustus 2026 menemukan implementasi awal cuma bandingkan bearer statis — tidak ada perlindungan replay dan tidak terikat ke isi body.
  - Verifikasi dipecah ke `_shared/webhookSignature.ts` (parsing header, cek toleransi waktu, hitung & bandingkan HMAC secara konstan-waktu) — modul murni Web Crypto (`crypto.subtle`), jadi test Vitest menjalankan kode produksi yang sama persis, bukan tiruan.
  - **Body dibaca sebagai teks mentah (`request.text()`) dulu, verifikasi dulu, baru `JSON.parse`.** RevenueCat menghitung signature dari byte body persis seperti dikirim; `request.json()` lalu `JSON.stringify` ulang bisa mengubah urutan key/spasi/escaping unicode dan menolak request yang sah. Diuji eksplisit: signature yang sah untuk body asli ditolak untuk body yang sudah diformat ulang dengan isi sama.
  - **Toleransi timestamp 5 menit** (default `toleranceSeconds = 300`, dua arah) — mencegah replay: request yang diambil dan dikirim ulang nanti (atau timestamp dipalsukan ke masa depan) ditolak walau signature-nya sah.
